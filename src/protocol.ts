...
export const SLOTS = { Jazz: 0, Pop: 1, Rock: 2, Dance: 3, USER1: 4, 'R&B': 5, Classic: 6, 'Hip-hop': 7, USER2: 8, USER3: 9, BYPASS: 10 };
export const USER_SLOTS = [4, 8, 9];
export const FILTER_TYPES = ['PK', 'LS', 'HS', 'BP', 'LP', 'HP', 'AP'] as const;
export type FilterType = typeof FILTER_TYPES[number];
export interface Band { type: FilterType; frequencyHz: number; gainDb: number; q: number }
export interface Eq { preampDb: number; bands: Band[] }
export interface State extends Eq { slot: number }
...
export function packet(write: boolean, command: number, data: number[] = []): Buffer {
  ...
  result.set([REPORT_ID, write ? 0xaa : 0xbb, write ? 0x0a : 0x0b, 0, 0, command, data.length, ...data, 0, 0xee]);
  return result;
}
...
export function response(report: Buffer): { command: number; data: Buffer } | undefined {
  ...
  if (report.length < 9 || report[3] !== 0 || report[4] !== 0) throw new Error('Malformed KA17 response');
  const length = report[6]!;
  if (length > 24 || report.length < length + 9 || report[7 + length] !== 0 || report[8 + length] !== 0xee) {
...
export function decodeBand(bytes: Buffer): Band {
  ...
  return { type, gainDb: bytes.readInt16BE(1) / 10, frequencyHz: bytes.readUInt16BE(3), q: bytes.readUInt16BE(5) / 100 };
}

export function difference(expected: Eq, actual: Eq): string[] {
  const differences: string[] = [];
  if (expected.preampDb !== actual.preampDb) differences.push(`preampDb: expected ${expected.preampDb}, got ${actual.preampDb}`);
  if (expected.bands.length !== actual.bands.length) differences.push(`band count: expected ${expected.bands.length}, got ${actual.bands.length}`);
  expected.bands.forEach((band, index) => {
    for (const key of ['type', 'frequencyHz', 'gainDb', 'q'] as const) {
      if (band[key] !== actual.bands[index]?.[key]) differences.push(`bands[${index}].${key}: expected ${band[key]}, got ${actual.bands[index]?.[key]}`);
    }
  });
  return differences;
}
