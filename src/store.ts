...
import { object, parseEq } from './preset.ts';
...
export async function withLock<T>(operation: () => Promise<T>, path = lockPath): Promise<T> {
  ...
  try { file = await open(path, 'wx', 0o600); }
    ...
    if ((error as NodeJS.ErrnoException).code === 'EEXIST') throw new Error(`Another aufio operation holds ${path}. If a previous process crashed, inspect its PID in this file and remove the stale lock only after confirming it has exited.`);
    throw error;
  }
  try {
...
export function presetPath(dataDir: string, name: string): string {
  if (!/^[a-z0-9][a-z0-9_-]{0,63}$/.test(name)) throw new Error('Preset name must be 1–64 lowercase letters, digits, hyphens or underscores');
  return join(dataDir, 'presets', `${name}.json`);
}

export function inputPath(dataDir: string, nameOrPath: string): string {
  return nameOrPath.endsWith('.json') || nameOrPath.includes('/') ? resolve(nameOrPath) : presetPath(dataDir, nameOrPath);
}

export async function listPresets(dataDir: string): Promise<string[]> {
  try { return (await readdir(join(dataDir, 'presets'))).filter(p => p.endsWith('.json')).sort(); }
  catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return []; throw error; }
}

export async function backup(dataDir: string, snapshot: Snapshot): Promise<string> {
  const path = join(dataDir, 'backups', `${snapshot.capturedAt.replaceAll(':', '-')}-${randomUUID()}.json`);
...
export function parseSnapshot(value: unknown): Snapshot {
  ...
  const identity = object(s.identity);
  const state = object(s.state);
  if (s.kind !== 'ka17-snapshot' || s.schemaVersion !== 1 || identity.vendorId !== VENDOR_ID || identity.productId !== PRODUCT_ID) throw new Error('Not a version-1 KA17 snapshot');
  if (typeof identity.release !== 'number' || typeof state.slot !== 'number' || !USER_SLOTS.includes(state.slot)) throw new Error('Snapshot must contain a USER slot and device revision');
  parseEq(state);
