...
import { bandBytes, COMMAND, decodeBand, difference, packet, PRODUCT_ID, response, signedGain, USER_SLOTS, VENDOR_ID } from './protocol.ts';
...
export async function devices(): Promise<HID.Device[]> {
  ...
  return [...new Map(found.filter(d => d.path && d.usagePage === 1).map(d => [d.path, d])).values()];
}
...
export async function openDevice(path?: string): Promise<{ device: Ka17; identity: HID.Device }> {
  ...
  if (found.length !== 1) throw new Error(`Expected one KA17 control interface, found ${found.length}. Run inspect; use --device when more than one is attached.`);
...
export class Ka17 {
  ...
  async send(write: boolean, command: number, data: number[] = []): Promise<void> {
  ...
  async get(command: number, length: number, index?: number): Promise<Buffer> {
    ...
    while (Date.now() < deadline) {
      ...
      if (!parsed || parsed.command !== command || (index !== undefined && parsed.data[0] !== index)) continue;
      if (parsed.data.length !== length) throw new Error(`Unexpected response length for command 0x${command.toString(16)}`);
      this.reports.push(report.toString('hex'));
      return parsed.data;
    }
    throw new Error(`KA17 response timeout for command 0x${command.toString(16)}. Close FiiO Control and retry the read; writes are not automatically retried.`);
  }

  async read(): Promise<State> {
    this.reports = [];
    const slot = (await this.get(COMMAND.slot, 1))[0]!;
    ...
    if ((await this.get(COMMAND.slot, 1))[0] !== slot) throw new Error('Preset changed during read. Close other controllers and retry.');
    return { slot, preampDb, bands };
  }

  async select(slot: number): Promise<void> {
    if (!USER_SLOTS.includes(slot) && slot !== 10) throw new Error('Only USER1, USER2, USER3 and BYPASS can be selected');
    await this.send(true, COMMAND.slot, [slot]);
    if ((await this.get(COMMAND.slot, 1))[0] !== slot) throw new Error('Preset selection did not match readback');
  }
  ...
  async apply(eq: Eq, before: State): Promise<State> {
    ...
    if (current.slot !== before.slot || difference(before, current).length) throw new Error('EQ changed since backup; refusing to overwrite it');
    ...
    for (const [index, band] of eq.bands.entries()) await this.send(true, COMMAND.band, bandBytes(index, band));
