...
import { bandBytes, decodeBand, packet, response, signedGain, SLOTS, USER_SLOTS } from '../src/protocol.ts';
...
test('golden packets use report ID 1, big-endian signed gains, exact Q and 32-byte payloads', () => {
  ...
  assert.equal(packet(true, 0x15, bytes).subarray(0, 17).toString('hex'), '01aa0a0000150809ffe9303901c80200ee');
  assert.deepEqual(decodeBand(Buffer.from('09ffe9303901c802', 'hex')), { type: 'HS', frequencyHz: 12345, gainDb: -2.3, q: 4.56 });
...
class Scripted implements Transport {
  writes: Buffer[] = [];
  replies: Buffer[];
  constructor(hex: string[]) { this.replies = hex.map(h => Buffer.from(h, 'hex')); }
  async write(data: Buffer) { this.writes.push(data); return data.length; }
  async read() { if (!this.replies.length) await delay(2); return this.replies.shift(); }
  async close() {}
}
...
test('read requires count, preamp, all indexed bands and an unchanged slot', async () => {
  ...
  assert.deepEqual(state, { slot: 8, preampDb: -7.6, bands: [
    { type: 'PK', gainDb: -1.3, frequencyHz: 2900, q: 3 },
    { type: 'HS', gainDb: 1, frequencyHz: 3000, q: 0.35 },
  ] });
  assert.deepEqual(transport.writes.map(p => p[5]), [0x16, 0x18, 0x17, 0x15, 0x15, 0x16]);
...
class Dsp implements Transport {
  state: State = { slot: 4, preampDb: -3.2, bands: [{ type: 'PK', frequencyHz: 731, gainDb: -2.1, q: 1.23 }] };
  writes: Buffer[] = [];
  replies: Buffer[] = [];
  ignoreBand = false;
  corruptSave = false;
  ...
  async write(p: Buffer) {
    ...
    if (p[1] === 0xaa) {
      ...
      if (command === 0x15 && !this.ignoreBand) this.state.bands[p[7]!] = {
        type: (['PK', 'LS', 'HS'] as const)[p[14]!]!,
        frequencyHz: p.readUInt16BE(10), gainDb: p.readInt16BE(8) / 10, q: p.readUInt16BE(12) / 100,
      };
      ...
      else if (command === 0x17) { data = Buffer.alloc(2); data.writeInt16BE(Math.round(this.state.preampDb * 10)); }
      else {
        const b = this.state.bands[p[7]!]!;
        data = Buffer.alloc(8); data[0] = p[7]!;
  ...
  async read() { return this.replies.shift(); }
  async close() {}
}

const target = { preampDb: -6.3, bands: [{ type: 'HS' as const, frequencyHz: 4321, gainDb: 2.4, q: 0.71 }] };
...
test('apply attenuates first, verifies before save, saves the correct slot, verifies after save', async () => {
  ...
  assert.deepEqual(changes.map(p => p[5]), [0x17, 0x18, 0x15, 0x17, 0x19]);
...
test('a dropped band write prevents saving; post-save corruption is also an error', async () => {
  const dsp = new Dsp(); dsp.ignoreBand = true;
  await assert.rejects(new Ka17(dsp, 0, 50).apply(target, structuredClone(dsp.state)), /readback mismatch/);
  assert.equal(dsp.writes.some(p => p[1] === 0xaa && p[5] === 0x19), false);
  assert.equal(dsp.state.preampDb, -12, 'Do not raise preamp after a failed band write');
  const corrupt = new Dsp(); corrupt.corruptSave = true;
  await assert.rejects(new Ka17(corrupt, 0, 50).apply(target, structuredClone(corrupt.state)), /preampDb/);
});
...
test('factory slots, stale backups, disconnects and short writes never proceed to save', async () => {
  ...
  await assert.rejects(device.apply(target, { ...dsp.state, preampDb: -9 }), /changed since backup/);
  ...
  const short = new Scripted([]); short.write = async () => 1;
