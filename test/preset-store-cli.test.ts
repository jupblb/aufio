...
import { fromDevice, headroom, magnitudeDb, parsePreset, toDevice } from '../src/preset.ts';
import { parseSnapshot, presetPath, readJson, saveJson, withLock } from '../src/store.ts';
import { difference } from '../src/protocol.ts';

const base = fromDevice('test', { preampDb: -7, bands: [{ type: 'PK', frequencyHz: 1000, gainDb: 6, q: 1 }] });
...
test('Q compensation is explicit, gain-sign symmetric, peaking-only and never silently clamped', () => {
  ...
  assert.throws(() => toDevice({ ...rbj, bands: [{ ...base.bands[0]!, q: 8 }] }), /refusing to clamp/);
...
test('validation rejects typos, nonfinite numbers, unsupported types and out-of-range/precision values', () => {
  ...
  for (const patch of [{ frequencyHz: 19 }, { frequencyHz: 20001 }, { q: 0.09 }, { q: 10.01 }, { gainDb: 12.1 }, { gainDb: -12.1 }, { gainDb: 1.01 }, { frequencyHz: 21.5 }, { q: NaN }, { gainDb: Infinity }, { type: 'notch' }, { frequncyHz: 100 }]) {
  ...
  assert.throws(() => toDevice(parsePreset({ ...base, bands: [{ ...base.bands[0], type: 'LP' }] })), /PK, LS, HS only/);
  const boundaries = { ...base, preampDb: -12, bands: [{ type: 'PK', frequencyHz: 20, gainDb: -12, q: 0.1 }, { type: 'HS', frequencyHz: 20000, gainDb: 12, q: 10 }] };
...
test('headroom uses the combined response, including overlapping bands and shelf resonance', () => {
  ...
  const overlapping = { preampDb: -5, bands: [{ ...base.bands[0]!, gainDb: 3 }, { ...base.bands[0]!, gainDb: 4 }] };
  ...
  assert.equal(headroom({ preampDb: 0, bands: [{ ...base.bands[0]!, gainDb: -8 }] }).estimatedPeakDb, 0);
});
...
test('diff catches type, count, preamp, frequency, gain and Q rather than just successful reads', () => {
  ...
  assert.equal(difference(base, { preampDb: -6, bands: [{ type: 'HS', frequencyHz: 999, gainDb: 5.9, q: 1.01 }] }).length, 5);
...
test('cross-process lock rejects overlap and releases after a failed operation', async () => {
  ...
  try {
    await withLock(async () => { await assert.rejects(withLock(async () => {}, lock), /Another aufio/); }, lock);
    await assert.rejects(withLock(async () => { throw new Error('failed'); }, lock), /failed/);
...
test('snapshot validation never treats a factory slot or foreign DAC as restorable', () => {
  const snapshot = { schemaVersion: 1, kind: 'ka17-snapshot', identity: { vendorId: 0x2972, productId: 0x93, release: 0x225 }, state: { slot: 4, preampDb: base.preampDb, bands: base.bands } };
  assert.equal(parseSnapshot(snapshot).state.slot, 4);
  assert.throws(() => parseSnapshot({ ...snapshot, state: { ...snapshot.state, slot: 7 } }), /USER slot/);
  assert.throws(() => parseSnapshot({ ...snapshot, identity: { ...snapshot.identity, productId: 0x88 } }), /KA17 snapshot/);
});

test('CLI errors are JSON and writes require confirmation before touching USB or files', () => {
  for (const args of [['apply', 'missing'], ['restore', 'missing'], ['select', 'USER1'], ['not-a-command']]) {
    const result = spawnSync(process.execPath, ['bin/aufio.mjs', '--device', 'NONEXISTENT', ...args], { encoding: 'utf8' });
